...
func cosine_similarity(list1 [26]float64, list2 [26]float64) float64 {
	var up float64 = 0.0
	for i := 0; i < len(list1); i++ {
		up += list1[i] * list2[i]
	}
	return up / (pythagorean_list(list1) * pythagorean_list(list2))
}

func pythagorean_list(list [26]float64) float64 {
...
func LanguageDetection(text string) string {
	...
	occurrence := getOccurance(text)

	var a = map[string]float64{"de": cosine_similarity(occurrence, de), "en": cosine_similarity(occurrence, en), "fr": cosine_similarity(occurrence, fr)}
...
func getOccurance(text string) [26]float64 {
